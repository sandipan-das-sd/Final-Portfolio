...
export async function listProjects(includeDrafts=false) { try{const sql=client();if(!sql)return defaultProjects;await ensureTables(sql);const rows=includeDrafts?await sql`SELECT * FROM portfolio_projects ORDER BY position,id`:await sql`SELECT * FROM portfolio_projects WHERE published=true ORDER BY position,id`;return rows.map(mapProject);}catch(error){console.error("Portfolio database unavailable; using built-in projects.",error);return defaultProjects;} }
