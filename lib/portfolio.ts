...
export const defaultProjects: PortfolioProject[] = [
  ...
  ["AI Text Classifier","AI-assisted classification API",["React","Node.js","Express","MongoDB","Ollama"],"A full-stack classification service that uses a local LLM to categorize customer messages as complaints, queries, feedback or other, returning a validated confidence score through a production REST API.","/projects/ai-text-classifier","/work/ai-text-classifier.png"],
...
const featureClassifier = (projects: PortfolioProject[]) => {
  const classifier = defaultProjects.find(project => project.title === "AI Text Classifier")!;
  return [classifier, ...projects.filter(project => project.title !== "AI Text Classifier" && !project.href.includes("ai-assisted-text-classification-api.vercel.app") && !project.href.includes("/projects/ai-text-classifier"))];
};
...
export async function listProjects(includeDrafts=false) { try{const sql=client();if(!sql)return featureClassifier(defaultProjects);await ensureTables(sql);const rows=includeDrafts?await sql`SELECT * FROM portfolio_projects ORDER BY position,id`:await sql`SELECT * FROM portfolio_projects WHERE published=true ORDER BY position,id`;let projects=rows.map(mapProject);const aaranya=defaultProjects.find(project=>project.title==="Aaranya")!;if(!projects.some(project=>project.title.toLowerCase()==="aaranya"||project.href.includes("aaranya.sandipandas.website")))projects=[aaranya,...projects];return featureClassifier(projects);}catch(error){console.error("Portfolio database unavailable; using built-in projects.",error);return featureClassifier(defaultProjects);} }
