...
export async function listProjects(includeDrafts=false) { try{const sql=client();if(!sql)return defaultProjects;await ensureTables(sql);const rows=includeDrafts?await sql`SELECT * FROM portfolio_projects ORDER BY position,id`:await sql`SELECT * FROM portfolio_projects WHERE published=true ORDER BY position,id`;let projects=rows.map(mapProject);const aaranya=defaultProjects[0];const classifier=defaultProjects.find(project=>project.title==="AI Text Classifier")!;if(!projects.some(project=>project.title.toLowerCase()==="aaranya"||project.href.includes("aaranya.sandipandas.website")))projects=[aaranya,...projects];if(!projects.some(project=>project.title==="AI Text Classifier"||project.href.includes("ai-assisted-text-classification-api.vercel.app")||project.href.includes("/projects/ai-text-classifier")))projects=[...projects,classifier];return projects;}catch(error){console.error("Portfolio database unavailable; using built-in projects.",error);return defaultProjects;} }
