...
export async function listProjects(includeDrafts=false) { const sql=client(); if(!sql) return defaultProjects; await ensureTables(sql); const rows=includeDrafts ? await sql`SELECT * FROM portfolio_projects ORDER BY position,id` : await sql`SELECT * FROM portfolio_projects WHERE published=true ORDER BY position,id`; return rows.map(mapProject); }
...
export async function getResumeUrl() { const sql=client(); if(!sql) return "/documents/Sandipan-Das-Resume.pdf"; await ensureTables(sql); const rows=await sql`SELECT value FROM portfolio_settings WHERE key='resume_url'`; return rows[0]?.value ? String(rows[0].value) : "/documents/Sandipan-Das-Resume.pdf"; }
...
export async function listMessages():Promise<PortfolioMessage[]>{const sql=client();if(!sql)return[];await ensureTables(sql);const rows=await sql`SELECT id,name,email,subject,message,received_at AS "receivedAt" FROM portfolio_messages ORDER BY received_at DESC LIMIT 500`;return rows.map(r=>({id:Number(r.id),name:String(r.name),email:String(r.email),subject:String(r.subject),message:String(r.message),receivedAt:new Date(String(r.receivedAt)).toISOString()}));}
